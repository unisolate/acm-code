...
struct circle {
    ...
    circle(point a, point b, point c)  //三角形的外接圆
    {
        p = line(a.add(b).div(2), a.add(b).div(2).add(b.sub(a).rotleft()))
                .crosspoint(line(c.add(b).div(2),
                                 c.add(b).div(2).add(b.sub(c).rotleft())));
        r = p.distance(a);
    }
...
struct polygon {
    ...
    void getconvex(polygon &convex) {
        ...
        for (i = 2; i < n; i++) {
            while (top &&
                   convex.p[top].sub(p[i]).det(convex.p[top - 1].sub(p[i])) <=
                       0)
                top--;
            convex.p[++top] = p[i];
        }
    ...
    void find(int st, point tri[], circle &c) {
        ...
        if (st == 2) {
            c = circle(tri[0].add(tri[1]).div(2),
                       tri[0].distance(tri[1]) / 2.0);
        }
        if (st == 3) {
                ...
                for (j = 0; j < p.size(); j++)
                    if (i != j) {
                        for (w = 0; w < p[j].n; w++) {
                            ...
                            if (c1 * c2 < 0)
                                ins(s, t, line(s, t).crosspoint(line(a, b)),
                                    -c2);
                            else if (!c1 && c0 * c2 < 0)
                                ins(s, t, a, -c2);
                            else if (!c1 && !c2) {
                                int c3 = dblcmp(t.sub(s).det(
                                    p[j].p[(w + 2) % p[j].n].sub(s)));
        ...
        for (i = 0; i < n; i++) {
            ...
            for (j = 0; j < v.size(); j++) {
                if (cur && dblcmp(v[j].first - pre[cur])) {
                    ...
                    ans[cur] +=
                        0.5 *
                        point(c[i].p.x + c[i].r * cos(pre[cur]),
    ...
    void unique() {
        ...
        for (i = 1; i < n; i++) {
            if (dblcmp(hp[i].angle - hp[i - 1].angle))
                hp[m++] = hp[i];
            else if (dblcmp(hp[m - 1]
                                .b.sub(hp[m - 1].a)
                                .det(hp[i].a.sub(hp[m - 1].a)) > 0))
                hp[m - 1] = hp[i];
        }
        n = m;
    }
    bool halfplaneinsert() {
        ...
        for (i = 2; i < n; i++) {
            while (st < ed &&
                ...
                ed--;
            while (st < ed &&
                   dblcmp((hp[i].b.sub(hp[i].a).det(p[st + 1].sub(hp[i].a)))) <
                       0)
                st++;
            que[++ed] = i;
...
struct line3 {
    ...
    bool pointonseg(point3 p) {
        return dblcmp(p.sub(a).det(p.sub(b)).len()) == 0 &&
    ...
    {
        if (dblcmp((p.sub(a).det(p.sub(b)).len())) == 0)
            return p;
...
struct plane {
    ...
    int pointontriangle(point3 p)  //点是否在空间三角形abc上
    {
        ...
        if (dblcmp(s - s1 - s2 - s3))
            return 0;
        if (dblcmp(s1) && dblcmp(s2) && dblcmp(s3))
            return 2;
        return 1;
    }
    //判断两平面关系
