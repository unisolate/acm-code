...
bool contain(int ida, int idb) {  // b是否为a的子串
                                  // ，ida和idb为字符串下标，若只有两个字符串，使用时传入参数(0,
                                  // 1)、(1, 0)就行
    if (len[ida] < len[idb])
        return false;
    ull hab = ha[idb][len[idb]];
    for (int i = 1; i + len[idb] <= len[ida]; ++i)
        if (get_hash(ha[ida], i, len[idb]) == hab)
            return true;
    return false;
}
...
int overlap(
    int ida,
    int idb) {  // 求a后缀与b前缀的最长公共子串，ida和idb为字符串下标，若只有两个字符串，使用时传入参数(0,1)、(1,
                // 0)就行
    int ans = 0, i;
    Forr(i, 1, min(len[ida], len[idb]) +
                   1) if (get_hash(ha[ida], len[ida] - i + 1, i) ==
                          get_hash(ha[idb], 1, i)) ans = i;
    // 可在if中加上 && strncmp(s[ida] + len[ida] - i + 1, s[idb] + 1, i) ==
    // 0(不过这就失去意义了，还不如双hash)
    return ans;
}
