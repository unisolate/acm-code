...
int main()
{
    ...
    int cas=0;
    while(scanf("%lf%lf%lf%lf%lf%lf%lf%lf",&x1,&y1,&x2,&y2,&x3,&y3,&x4,&y4)&&x1+x2+x3+x4+y1+y2+y3+y4)
    {
        circles cs;cs.n=0;
        circle c1,c2,c3,c4;
        c1.p.x=(x1+x3)/2,c1.p.y=(y1+y3)/2,c1.r=sqrt((x1-x3)*(x1-x3)+(y1-y3)*(y1-y3))/2;
        c2.p.x=(x1+x4)/2,c2.p.y=(y1+y4)/2,c2.r=sqrt((x1-x4)*(x1-x4)+(y1-y4)*(y1-y4))/2;
        c3.p.x=(x2+x3)/2,c3.p.y=(y2+y3)/2,c3.r=sqrt((x2-x3)*(x2-x3)+(y2-y3)*(y2-y3))/2;
        c4.p.x=(x2+x4)/2,c4.p.y=(y3+y4)/2,c4.r=sqrt((x2-x4)*(x2-x4)+(y2-y4)*(y2-y4))/2;
        cs.add(c1);
        cs.add(c2);
        cs.add(c3);
        cs.add(c4);
