...
struct point {
    ...
    bool operator<(point a)const {
        return dblcmp(a.y - y) == 0 ? dblcmp(x - a.x) < 0 : y < a.y;
    }
    double len() {
        return hypot(x, y);
    }
...
struct circle {
    ...
    circle(point a, point b, point c) { //三角形的外接圆
        p = line(a.add(b).div(2),
                 a.add(b).div(2).add(b.sub(a).rotleft())).crosspoint(line(c.add(b).div(2),
    ...
    double areatriangle(point a, point b) {
        ...
        q[len++] = b;
        if (len == 4
                && (dblcmp(q[0].sub(q[1]).dot(q[2].sub(q[1]))) > 0))swap(q[1], q[2]);
        double res = 0;
        int i;
        for (i = 0; i < len - 1; i++) {
...
int main() {
    ...
    while (t--) {
        ...
        b.input();
        a.adjust(); b.adjust();
        // a.crosspoint(b).output();
        if (a.segcrossseg(b) == 0 || dblcmp(a.a.y - a.b.y) == 0
                || dblcmp(b.a.y - b.b.y) == 0) {
            puts("0.00");
        } else {
            double a1 = b.b.sub(b.a).det(a.a.sub(b.a));
            double a2 = b.b.sub(b.a).det(a.b.sub(b.a));
            if (dblcmp(a2 - a1) == 0) {
            	puts("0.00");
            	continue;
            }
            point c = a.crosspoint(b), d, f;
            if (a.b < b.b) {
                if (a.angle() < pi / 2 && b.angle() < pi / 2 && a.angle() < b.angle()
                        && a.b.x <= b.b.x) {
                    // printf("%.2lf %.2lf\n", a.angle(), b.angle());
                    // a.b.output(); b.b.output();
                    puts("0.00");
                    continue;
                }
                if (a.angle() > pi / 2 && b.angle() > pi / 2 && a.angle() > b.angle()
                ...
                if (a.angle() < pi / 2 && b.angle() < pi / 2 && b.angle() < a.angle()
                        && b.b.x <= a.b.x) {
                    // printf("%.2lf %.2lf\n", a.angle(), b.angle());
                    // a.b.output(); b.b.output();
                    puts("0.00");
