...
int main() {
    while (scanf("%d", &n) && n) {
        ...
        l[0].a.x = xl, l[0].a.y = yl;
        l[0].b.x = xl, l[0].b.y = yr;
        l[n + 1].a.x = xr, l[n + 1].a.y = yl;
        l[n + 1].b.x = xr, l[n + 1].b.y = yr;
        for (int i = 0; i < n; ++i) {
            scanf("%d%d", &b[i].u, &b[i].v);
        }
        sort(b, b + n, cmp);
        for (int i = 0; i < n; ++i) {
            l[i + 1].a.x = b[i].u, l[i + 1].a.y = yl;
            l[i + 1].b.x = b[i].v, l[i + 1].b.y = yr;
        }
        while (m--) {
            scanf("%d%d", &x, &y);
            p.x = x, p.y = y;
            for (int i = 0; i <= n; ++i) {
                if (l[i].relation(p) == 1 && l[i + 1].relation(p) == 2) {
                    ++a[i];
                    break;
                }
            }
