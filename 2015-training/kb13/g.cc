...
int main() {
    ...
    for (int i = 0; i < n; ++i) {
        l[i].input();
        p[i * 2] = l[i].a, p[i * 2 + 1] = l[i].b;
    }
    // p[n * 2] = point(0, 0);
    // p[n * 2 + 1] = point(0, 100);
    // p[n * 2 + 2] = point(100, 0);
    // p[n * 2 + 3] = point(100, 100);
    // l[n] = line(p[n * 2], p[n * 2 + 1]);
    // l[n] = line(p[n * 2], p[n * 2 + 2]);
    // l[n] = line(p[n * 2 + 1], p[n * 2 + 3]);
    // l[n] = line(p[n * 2 + 2], p[n * 2 + 3]);
    point t; t.input();
    if (n == 0) {
        a = 0;
    } else {
