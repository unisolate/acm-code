...
struct High_Num {
    ...
    inline bool operator<(const High_Num& b) {
        return (nag != b.nag) ? nag
                              : (nag ? bigger(*this, b) : smaller(*this, b));
    }
    inline bool operator>(const High_Num& b) {
        return (nag != b.nag) ? b.nag
    ...
    inline High_Num operator+(const int t) {
        High_Num t2 = t;
        return (*this) + t2;
    }
    inline High_Num operator-(const int t) {
